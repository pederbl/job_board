class GeonamesLocation
  ...
  field :is_country, type: Boolean
  field :country_geonameid, type: Integer
  ...
  def self.search(attrs)
    ...
    if attrs[:country_code].present?
      ...
      client.filters << Riddle::Client::Filter.new("is_country", [true]) 
    ...
    if attrs[:admin1].present?
      ...
      client = sphinx_client(attrs)
      client.filters << Riddle::Client::Filter.new("country_geonameid", countries_geonameids) if countries_geonameids.present?
      results = client.query(query, "geonames_locations")
    ...
    if attrs[:admin2].present?
      ...
      client = sphinx_client(attrs)
      client.filters << Riddle::Client::Filter.new("country_geonameid", countries_geonameids) if attrs[:country] or attrs[:country_code]
  ...
  def self.refresh_countries_list
    Value.set(:countries, where(is_country: true).map { |d| 
      { 
  ...
  def self.xmlpipe_main
    ...
    xml.sphinx:docset do |docset| 
      xml.sphinx:schema do |schema|
        ...
        schema.send("sphinx:attr", name: "is_country", type: "bool")
      ...
      ids.each_with_index { |id, i|
        ...
        docset.send("sphinx:document", id: i + 1) do |doc|
          ...
          doc.is_country(d.is_country ? 1 : 0)
  ...
  def self.load_all_countries_file(path)
    ...
    lines.each_with_index { |line, i|
      ...
      next if arr[7] != "ADM1"
      loc = new_from_file_array(arr)
      loc.country_geonameid = where(country_code: loc.country_code, is_country: true).first.geonameid 
